// Este script implementa un sistema de cambio de tema (claro/oscuro) de forma automática. Inyecta un botón flotante en la página que permite al usuario alternar entre los modos, guarda la preferencia del usuario y se adapta a la configuración del sistema operativo.
/**
...
// Objeto que mapea nombres de colores CSS a sus valores hexadecimales. Se utiliza para poder procesar colores nombrados como 'red', 'blue', etc.
const COLOR_MAP = {
...
export class AutoTheme {
  ...
  #isDark = false; // Estado actual del tema (true = oscuro, false = claro).
  ...
  constructor(baseColors, buttonOptions = {}) {
    ...
    // Asigna los colores base o usa valores por defecto si no se proporcionan.
    this.#baseColors = baseColors ?? { surface: '#ffffff', text: '#2c3e50' };
    this.#validateColors();

    // Fusiona las opciones proporcionadas por el usuario con las opciones por defecto.
    this.#buttonOptions = {
  ...
  /**
   * Valida que los colores base proporcionados tengan un formato HEX válido.
   * Si un color es inválido, lo reemplaza por un valor por defecto seguro.
   */
  #validateColors() {
  ...
  /**
   * Convierte un color en formato hexadecimal (HEX) a formato HSL (Tono, Saturación, Luminosidad).
   * Esto es crucial para poder invertir la luminosidad y generar el tema oscuro.
   * @param {string} hex - El color en formato #RRGGBB o #RGB.
  ...
  #hexToHsl(hex) {
    if (hex) {
      ...
      // Convierte los componentes R, G, B a un rango de 0 a 1.
      ...
      // Devuelve los valores HSL redondeados.
      return { h: Math.round(h * 360), s: Math.round(s * 100), l: Math.round(l * 100) };
    }
    console.error(`AutoTheme Error: Invalid HEX color provided to #hexToHsl: ${hex}`);
    return { h: 0, s: 0, l: 100 }; // Devuelve blanco como valor por defecto en caso de error.
  }

  /**
   * Genera las versiones "oscuras" de los colores base.
   * Lo hace invirtiendo el valor de luminosidad (L) del HSL.
   * También ajusta la saturación (S) para mejorar el contraste y la apariencia.
   * @returns {object} - Un objeto con los colores oscuros generados.
   */
  #generateDarkColors() {
    const darkColors = {};
  ...
  /**
   * Obtiene el color de fondo "efectivo" de la página.
   * Recorre el DOM hacia arriba desde el `body` hasta encontrar un elemento
   * con un color de fondo que no sea transparente.
   */
  ...
  /**
   * Convierte un nombre de color (ej. "red") a su valor hexadecimal usando el COLOR_MAP.
   * @param {string} color - El nombre del color.
   * @returns {string|null} - El valor HEX o null si no se encuentra.
   */
  ...
  /**
   * Parsea una cadena de color (rgb, hex, hsl, nombre) y devuelve sus componentes R, G, B.
   * @param {string} colorString - La cadena de color.
  ...
  #parseColorString(colorString) {
    ...
    if (this.#colorCache.has(sanitizedColor)) {
      return this.#parseColorString(this.#colorCache.get(sanitizedColor));
    }
    ...
    // Formato hex largo
    if ((match = sanitizedColor.match(/#([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})/))) {
      const [, r, g, b] = match.map(c => parseInt(c, 16));
      return { r, g, b };
    }
    // Formato hex corto
    if ((match = sanitizedColor.match(/#([a-f\d])([a-f\d])([a-f\d])/))) {
      const [, r, g, b] = match.map(c => parseInt(c + c, 16));
      return { r, g, b };
    }
    // Formato hsl(a)
    if ((match = sanitizedColor.match(/hsla?\((\d+),\s*(\d+)%?,\s*(\d+)%?/))) {
      ...
      return { r: Math.round((r_temp + m) * 255), g: Math.round((g_temp + m) * 255), b: Math.round((b_temp + m) * 255) };
    }
    // Nombre de color
    const hexColor = this.#colorNameToHex(sanitizedColor);
    if (hexColor) {
      this.#colorCache.set(sanitizedColor, hexColor);
      return this.#parseColorString(hexColor);
    }
  ...
  /**
   * Determina si un color es "oscuro" calculando su luminancia percibida.
   * @param {string} colorString - El color en cualquier formato (rgb, hex, hsl, nombre).
   * @returns {boolean} - True si el color es oscuro.
   */
  #isColorDark(colorString) {
    const rgb = this.#parseColorString(colorString);
    if (!rgb) return false; // No se puede determinar si no se puede parsear.

    // Fórmula estándar para calcular la luminancia percibida por el ojo humano.
    // Un valor por debajo de 128 se considera oscuro.
    const luminance = (0.299 * rgb.r + 0.587 * rgb.g + 0.114 * rgb.b);
    return luminance < 128;
  }

  /**
   * Vuelve a calcular si el tema base de la página es claro u oscuro.
   * Es útil si el fondo de la página cambia dinámicamente.
   */
   ...
   * @param {boolean} isDark - True para activar el modo oscuro, false para el claro.
   * @param {boolean} manualToggle - True si el cambio fue iniciado por el usuario (click en el botón).
   */
  setTheme(isDark, manualToggle = false) {
    try {
      // Evita reaplicar el mismo tema si no hay cambios.
      if (this.#isDark === isDark && document.documentElement.hasAttribute('data-theme-initialized')) return;

      this.#isDark = isDark;
      
      // Añade una clase al botón para disparar la animación del icono.
  ...
  /**
   * Lógica interna para aplicar los cambios de tema en el DOM.
   * @param {boolean} manualToggle - Indica si el cambio fue manual.
   */
  #applyThemeChanges(manualToggle) {
    ...
    // Marca que el tema ha sido inicializado.
    document.documentElement.setAttribute('data-theme-initialized', 'true');

    // Dispara un evento global para que otros scripts puedan reaccionar al cambio de tema.
  ...
  #getSavedButtonPosition() {
    try {
      ...
      if (typeof savedPosition.left === 'number' && typeof savedPosition.top === 'number') {
        return { left: `${savedPosition.left}px`, top: `${savedPosition.top}px` };
      }
    } catch (error) {
  ...
  /**
   * Implementa la funcionalidad de arrastrar y soltar para un elemento.
   * @param {HTMLElement} element - El elemento que se hará arrastrable.
   */
  ...
  /**
   * Inyecta un elemento `div` oculto con `aria-live="polite"` para la accesibilidad.
   * Los lectores de pantalla anunciarán cualquier cambio en el contenido de este div.
   */
  #injectLiveRegion() {
  ...
  /**
   * Actualiza el contenido (icono) y los colores del botón de tema
   * para que coincidan con el tema actual.
   */
  #updateToggleButton() {
    if (!this.#toggleButton) return;
    // Muestra el icono de luna si el tema es oscuro, y el de sol si es claro.
  ...
  #generateAllStyles() {
    ...
    return `
      ...
      /* Regla principal: cuando <html> tiene data-theme="inverted", se aplica el filtro. */
      [data-theme="inverted"] {
        /* Invierte todos los colores, rota el matiz para corregir colores (ej. azules se vuelven amarillos) y ajusta contraste/brillo. */
  ...
  #generateUIAndAnimationStyles() {
    ...
    return `
      ...
      #theme-tutorial-tooltip {
        position: absolute;
        bottom: 85px; /* Posicionado encima del botón central. Usamos un valor fijo para compatibilidad con JSDOM. */
        left: 50%;
      ...
      /* Cuando el tutorial está activo, detenemos la animación de los satélites. */
      .tutorial-active .theme-satellite-button { animation: none !important; }
    `;
  }
  ...
  #initializeModalObserver() {
      ...
      for (const mutation of mutations) {
        if (mutation.type === 'childList') {
          mutation.addedNodes.forEach(node => {
            // Solo nos interesan los HTMLElements.
            if (node.nodeType !== Node.ELEMENT_NODE) return;

            // Evitar que el contenedor del botón se detecte a sí mismo.
            if (node.id === 'theme-toggle-container') return;

            // Comprueba si el nodo parece un modal.
            // Criterios: posición fija y un z-index alto (típico de modales, pop-ups, diálogos).
            const style = window.getComputedStyle(node);
            const zIndex = parseInt(style.zIndex, 10);
            const isLikelyModal = style.position === 'fixed' && zIndex > 1000;
    ...
    this.#modalObserver.observe(document.body, {
      ...
      subtree: true    // Observar también en los descendientes.
    });
  }
